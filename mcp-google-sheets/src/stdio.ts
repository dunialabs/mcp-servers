...
let mcpServer: GoogleSheetsMcpServer | null = null;

async function main() {
  logger.info('[STDIO] Starting Google Sheets MCP Server');

  try {
    mcpServer = new GoogleSheetsMcpServer();
    await mcpServer.initialize();

    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('[STDIO] Google Sheets MCP Server connected via STDIO');
  } catch (error) {

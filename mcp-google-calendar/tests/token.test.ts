...
describe('google calendar token helpers', () => {
  ...
  it('normalizes optional Bearer prefix and whitespace', () => {
    expect(normalizeAccessToken('  Bearer ya29.example_token  ')).toBe('ya29.example_token');
  });

  it('reads normalized accessToken from env', () => {
    process.env.accessToken = '  Bearer ya29.example_token  ';
    expect(getCurrentToken()).toBe('ya29.example_token');
  });

...
class Logger {
  ...
  private redactSensitiveData(data: any, visited: WeakSet<object> = new WeakSet()): any {
    if (typeof data === 'string') {
      ...
      redacted = redacted.replace(/ya29\.[a-zA-Z0-9._-]+/g, 'ya29.***REDACTED***');

      // Redact Bearer tokens
      redacted = redacted.replace(/Bearer\s+[a-zA-Z0-9._-]+/g, 'Bearer ***REDACTED***');

      // Redact authorization headers
      redacted = redacted.replace(/(authorization|x-user-access-token):\s*[^\s]+/gi, '$1: ***REDACTED***');

      // Redact email addresses (keep first char + domain)
      redacted = redacted.replace(/\b([a-zA-Z0-9])[a-zA-Z0-9._-]*@([a-zA-Z0-9.-]+)/g, '$1***@$2');

      // Redact long event IDs in resource URIs (keep first 10 chars)
      redacted = redacted.replace(/gcalendar:\/\/\/([a-zA-Z0-9_-]{10})[a-zA-Z0-9_-]{10,}/g, 'gcalendar:///$1***');

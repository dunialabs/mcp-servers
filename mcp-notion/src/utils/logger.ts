...
class Logger {
  ...
  private redactSensitiveData(data: any, visited: WeakSet<object> = new WeakSet()): any {
    if (typeof data === 'string') {
      ...
      // Redact Notion tokens (secret_*)
      redacted = redacted.replace(/secret_[a-zA-Z0-9]+/g, 'secret_***REDACTED***');

      // Redact Bearer tokens
      redacted = redacted.replace(/Bearer\s+[a-zA-Z0-9._-]+/g, 'Bearer ***REDACTED***');

      // Redact authorization headers
      redacted = redacted.replace(/(authorization|notion-version):\s*[^\s]+/gi, '$1: ***REDACTED***');

      // Redact email addresses (keep first char + domain)
      redacted = redacted.replace(/\b([a-zA-Z0-9])[a-zA-Z0-9._-]*@([a-zA-Z0-9.-]+)/g, '$1***@$2');

      // Redact Notion page/block IDs (keep first 10 chars)
      redacted = redacted.replace(/\b([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-)[a-f0-9]{12}\b/g, '$1***');

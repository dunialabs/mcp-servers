...
function render(payload: DatabasePayload) {
  ...
  root.innerHTML = `
    <style>
      ...
      .table-head, .table-row { display:grid; grid-template-columns:minmax(180px,1.8fr) repeat(${Math.max(columns.length, 1)}, minmax(120px, 1fr)); gap:12px; align-items:center; }
          ...
          ${results.length === 0 ? '<div class="empty">No database rows matched this request.</div>' : results.map((item) => `
            <div class="table-row">
              <div>
                ...
                <div class="muted">${item?.url ? `<a href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer">Open in Notion</a>` : ''}</div>
              </div>

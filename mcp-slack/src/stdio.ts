...
let mcpServer: SlackMcpServer | null = null;

async function main() {
  logger.info('[STDIO] Starting Slack MCP Server');

  try {
    mcpServer = new SlackMcpServer();
    await mcpServer.initialize();

    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('[STDIO] Slack MCP Server connected via STDIO');
  } catch (error) {

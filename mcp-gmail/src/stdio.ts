...
let mcpServer: GmailMcpServer | null = null;

async function main() {
  logger.info('[STDIO] Starting Gmail MCP Server');

  try {
    mcpServer = new GmailMcpServer();
    await mcpServer.initialize();

    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('[STDIO] Gmail MCP Server connected via STDIO');
  } catch (error) {

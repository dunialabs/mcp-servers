...
let mcpServer: HubSpotMcpServer | null = null;

async function main() {
  logger.info('[STDIO] Starting HubSpot MCP Server');

  try {
    mcpServer = new HubSpotMcpServer();
    await mcpServer.initialize();

    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('[STDIO] HubSpot MCP Server connected via STDIO');
  } catch (error) {

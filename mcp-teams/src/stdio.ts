...
let mcpServer: TeamsMcpServer | null = null;

async function main() {
  logger.info('[STDIO] Starting Teams MCP Server');

  try {
    mcpServer = new TeamsMcpServer();
    await mcpServer.initialize();

    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('[STDIO] Teams MCP Server connected via STDIO');
  } catch (error) {
